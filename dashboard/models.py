...
class Indicator(models.Model):
    ...
    mitigation_plan = models.TextField("Plan de mitigation", blank=True)
    unit = models.CharField("Unité", max_length=20, blank=True)  # e.g. "%", "days"
    created_at = models.DateTimeField("Créé le", auto_now_add=True)

    @property
    def previous_value(self):
        ms = list(self.measurements.all()[:2])  # uses prefetch cache when present
        return ms[1].value if len(ms) > 1 else None

    @property
    def trend_value(self):
        current, previous = self.current_value, self.previous_value
    ...
    @property
    def status(self):
        """Maps to the CSS status-success/warning/danger classes used in the templates."""
        if self.kind == "KRI":
        ...
        mapping = {
            ...
            "far_below_target": "danger",
            "unknown": "unknown",
        }
        return mapping.get(self.performance_status, "unknown")

    def get_status_display(self):
        # not auto-generated by Django because `status` is a property, not a real field
        labels = {
            "success": "Normal" if self.kind == "KRI" else "On Target",
            "warning": "Warning",
            "danger": "Critical" if self.kind == "KRI" else "Far Below Target",
            "unknown": "No Data",
        }
